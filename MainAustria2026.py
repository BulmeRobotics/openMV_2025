...
cam_debug_mode = False   #Start debugging mode, skipping all resets
message_ok = "<OK>"
status = False
...
def check_start():
    ...
    SerialMessage = SerialRecieve()

    if SerialMessage == b"E":
        uart.write(message_ok)
        print("Enable recieved")
        run = True

    elif SerialMessage == b"D":
        uart.write(message_ok)
        run = False
...
def SerialRecieve():
    if uart.any() > 0:
        ...
        if raw_data:
            #print(raw_data)
            # Deine Logik für das Parsen von <Befehl>
            if ">" in raw_data:
                start = raw_data.find(b"<")
                end = raw_data.find(b">")
                if start != -1 and end > start:
                    print(raw_data[start + 1:end])
                    return raw_data[start + 1:end]
                    #raw_data = ""

def check_init():
    print("Checks Init")
    if SerialRecieve() == b"I":
            print("Init recieved")
            initDone = True
            return initDone
...
#Init Loop
while True:
    print("Init Loop enter")
    status = check_init()
    if status == True:
        uart.write(message_ok)
        print(message_ok + "OK, Loop break")
        break
...
while True:
    ...
    if (reset_pin.value() and init_done):
        cams_init()
    ...
    if run == True and stop_reset == False:
        ...
        if (frame_counter >= 7):
            ...
            if (center_positon_x <= max_detection_val_right and center_positon_x >= max_detection_val_left):
                uart.write("<" + CamTransmit + ">")             #Send Object via UART
        ...
        #print(time.ticksdiff(time.ticks_ms(), penis))
